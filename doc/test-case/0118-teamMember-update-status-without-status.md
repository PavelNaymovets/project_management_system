# 118: Обновление статуса участника проекта без указания статуса
...
* Выполнится HTTP запрос с методом `PUT` на endpoint `http://localhost:8080/pms/api/v1/team/member/role?id=dfdsf&status=`.
